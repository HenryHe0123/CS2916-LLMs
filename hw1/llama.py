...
class RMSNorm(torch.nn.Module):
    ...
    def _norm(self, x):
        ...
        # debug: self.weight is used in the forward pass
        return x / rms
...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        """
        #
        mul = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(self.head_dim)
...
class LlamaLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = self.attention(norm_x)
        x2 = x + attn
        norm_x2 = self.ffn_norm(x2)
        ffn = self.feed_forward(norm_x2)
        return x2 + ffn
